...
public final class EncryptorUtil {
    ...
    public static final String decrypt(byte[] key, byte[] cipherText, CipherEngineEnum cipherEngineEnum) throws InvalidCipherTextException, UnsupportedEncodingException {
        ...
        byte[] byteArray = new byte[cipher.getOutputSize(cipherText.length)];

        int oLen = cipher.processBytes(cipherText, 0, cipherText.length, byteArray, 0);
        cipher.doFinal(byteArray, oLen);
     ...
     *
     * @param keySize   an integer specifying key size in bits, should be one of the 128, 192, 256
